...
If the repo is small, single-author, and low-stakes, prefer `scrutinizing-projects` (single-pass) outright — see *When NOT to use this skill*.
...
- **Quick critique with one output file** — use `scrutinizing-projects`
...
- `superpowers:writing-skills` & `advanced-skill-authoring` — how this skill was authored
- `scrutinizing-projects` — lighter alternative
