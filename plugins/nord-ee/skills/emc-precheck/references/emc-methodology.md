...
When `--spice-enhanced` is passed and a SPICE simulator is detected (via the `spice-sim` skill's `detect_simulator()`), two checks use AC analysis for improved accuracy:
