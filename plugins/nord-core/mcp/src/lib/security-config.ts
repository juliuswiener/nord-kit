 ...
 *
 * Single entry point for all NORD security settings.
 * Two layers of configuration:
 *
 * 1. NORD_SECURITY env var — master switch
 *    - "strict": all security features enabled
 *    - unset/other: per-feature defaults apply
 *
 * 2. Config file (.claude/nord.jsonc or ~/.config/claude-nord/config.jsonc)
 *    security section — granular overrides (highest precedence)
 *
 * Precedence: config file > NORD_SECURITY env var > defaults (all off)
 */
...
export interface SecurityConfig {
  ...
  /** Disable project-level .nord/skills/ loading */
...
function loadSecurityFromConfigFiles(): Partial<SecurityConfig> {
  const paths = [
    join(process.cwd(), ".claude", "nord.jsonc"),
    join(getConfigDir(), "claude-nord", "config.jsonc"),
  ];
 ...
 * Precedence: config file > NORD_SECURITY env > defaults
 */
