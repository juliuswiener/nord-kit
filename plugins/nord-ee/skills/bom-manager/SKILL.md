...
| Skill | Purpose |
|-------|---------|
| `kicad-analyze` | Read/analyze schematics, PCB, footprints |
| `digikey-search` | Search DigiKey, download datasheets (primary prototype source) |
| `mouser-search` | Search Mouser (secondary prototype source) |
| `lcsc-search` | Search LCSC (production/JLCPCB parts) |
| `element14-search` | Search Newark/Farnell/element14 (international) |
| `jlcpcb-fab` | PCB fabrication & assembly ordering |
| `pcbway-fab` | Alternative PCB fab & assembly |
...
- [ ] Design rules meet manufacturer minimums (see `jlcpcb-fab` or `pcbway-fab` skill)
...
The `kicad-analyze` skill also creates analyzer JSON and design review markdown reports with user-chosen filenames — see its "Generated Files" section for tracking and cleanup guidance.
