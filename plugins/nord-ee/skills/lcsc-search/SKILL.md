...
LCSC is JLCPCB's sister company — they share the same parts library and `Cxxxxx` part numbers. Use LCSC for **production sourcing** (assembled boards from JLCPCB/PCBWay). DigiKey/Mouser are for prototyping. For BOM management and export workflows, see `bom-manager`.
...
- BOM matching in JLCPCB assembly (see `jlcpcb-fab` skill)
...
Key fields:
- `lcsc-search` — numeric LCSC ID (without "C" prefix)
