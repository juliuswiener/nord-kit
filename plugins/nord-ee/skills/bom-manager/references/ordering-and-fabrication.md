...
Same format for JLCPCB assembly orders — see the `jlcpcb-fab` skill.
...
Always query current pricing via the `digikey-search`, `mouser-search`, and `lcsc-search` skills — prices change frequently.
