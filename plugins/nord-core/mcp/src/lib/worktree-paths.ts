 ...
 *
 * Provides strict path validation and resolution for .nord/ paths,
 * ensuring all operations stay within the worktree boundary.
 *
 * Supports NORD_STATE_DIR environment variable for centralized state storage.
 * When set, state is stored at $NORD_STATE_DIR/{project-identifier}/ instead
 * of {worktree}/.nord/. This preserves state across worktree deletions.
 */
 ...
 * the NORD anchor regardless of git status — enables multi-repo workspaces
 ...
 *
 * Resolution order in getNordRoot(): NORD_STATE_DIR > workspace marker > git > cwd.
 */
export const WORKSPACE_MARKER = '.nord-workspace';

/** Standard .nord subdirectories */
export const NordPaths = {
  ROOT: '.nord',
  STATE: '.nord/state',
...
export function findWorkspaceRoot(startDir?: string): string | null {
  if (process.env.NORD_DISABLE_MULTIREPO === '1') return null;
  ...
  while (true) {
    // Stop before scanning $HOME (or above) so a stray ~/.nord-workspace does
 ...
 * `git rev-parse --show-toplevel` stops at the submodule and `.nord/` would be
 ...
 * outermost superproject so `.nord/` anchors to the monorepo root rather than
 * the submodule (#3349).
 *
 ...
 * literal `.nord` base; resolving such a subdir up to the repo root would
 ...
 * working tree so `.nord/` state anchors to the monorepo root rather than
...
// ============================================================================
// NORD_STATE_DIR SUPPORT (Issue #1014)
// ============================================================================
...
/**
 * Scan sibling subdirs of a workspace anchor for pre-existing .nord/state/ content.
 ...
 * getNordRoot() invocation to keep the hot path free of readdirSync calls.
 */
export function warnSiblingRetrofit(workspaceAnchor: string, sessionId?: string): void {
  ...
  const sharedNord = join(workspaceAnchor, NordPaths.ROOT);
  if (sessionId) {
    const markerPath = join(sharedNord, 'state', `sibling-retrofit-warned-${sessionId}.json`);
  ...
  for (const entry of entries) {
    ...
    const siblingStateDir = join(workspaceAnchor, entryName, NordPaths.ROOT, 'state');
    if (existsSync(siblingStateDir)) {
      legacyDirs.push(join(workspaceAnchor, entryName, NordPaths.ROOT));
    }
  }
  ...
  process.stderr.write(
    `[nord] workspace-retrofit warning: .nord-workspace anchor found at ${workspaceAnchor}\n` +
    `  but sibling repos have pre-existing local .nord/state/ content:\n${dirList}\n` +
    `  Shared state will go to: ${sharedNord}\n` +
    `  To migrate legacy state: NORD_MIGRATE_LEGACY_STATE=1 nord setup\n` +
    `  Or manually copy state files to ${sharedNord}/state/\n`
  );

  // Write disk marker so subsequent hook firings in the same session stay silent
  if (sessionId) {
    try {
      const stateDir = join(sharedNord, 'state');
 ...
 * Also removes any disk markers under the given nordStateDir when provided.
 * @internal
 */
export function clearSiblingRetrofitWarnings(nordStateDir?: string): void {
  siblingRetrofitWarned.clear();
  if (nordStateDir) {
    try {
      const stateDir = join(nordStateDir, 'state');
...
export function getProjectIdentifier(worktreeRoot?: string): string {
  ...
  // project identifier is a state *identity* (used for NORD_STATE_DIR centralized
  ...
  // test. The #3349 climb applies only to the on-disk `.nord/` *location*
  // (getNordRoot's default branch), not to identity. The no-arg fallback uses
  ...
  // below sees the unclimbed root so an inner `.nord-workspace` marker is honored.
...
/**
 * Get the .nord root directory path.
 *
 * When NORD_STATE_DIR is set, returns $NORD_STATE_DIR/{project-identifier}/
 * instead of {worktree}/.nord/. This allows centralized state storage that
 * survives worktree deletion.
 *
 * @param worktreeRoot - Optional worktree root
 * @returns Absolute path to the nord root directory
 */
export function getNordRoot(worktreeRoot?: string): string {
  const customDir = process.env.NORD_STATE_DIR;
  if (customDir) {
    // Centralized state lives at $NORD_STATE_DIR/{projectId} — outside the
    // working tree — so the #3349 stray-`.nord`-in-submodule problem does not
    ...
    // Log notice if both legacy .nord/ and new centralized dir exist
    const legacyPath = join(root, NordPaths.ROOT);
    const warningKey = `${legacyPath}:${centralizedPath}`;
    if (!dualDirWarnings.has(warningKey) && existsSync(legacyPath) && existsSync(centralizedPath)) {
      dualDirWarnings.add(warningKey);
      console.warn(
        `[nord] Both legacy state dir (${legacyPath}) and centralized state dir (${centralizedPath}) exist. ` +
  ...
  // share the same .nord/ at the marker location.
  const workspaceAnchor = findWorkspaceRoot(worktreeRoot);
  if (workspaceAnchor) {
    return join(workspaceAnchor, NordPaths.ROOT);
  }

  const root = resolveStateAnchorRoot(worktreeRoot);
  return join(root, NordPaths.ROOT);
}

/**
 * Resolve a relative path under .nord/ to an absolute path.
 * Validates the path is within the nord boundary.
 *
 * @param relativePath - Path relative to .nord/ (e.g., "state/ralph.json")
 * @param worktreeRoot - Optional worktree root (auto-detected if not provided)
 * @returns Absolute path
 * @throws Error if path would escape nord boundary
 */
export function resolveNordPath(relativePath: string, worktreeRoot?: string): string {
  validatePath(relativePath);

  const nordDir = getNordRoot(worktreeRoot);
  const fullPath = normalize(resolve(nordDir, relativePath));

  // Verify resolved path is still under nord directory
  const relativeToNord = relative(nordDir, fullPath);
  if (relativeToNord.startsWith('..') || relativeToNord.startsWith(sep + '..')) {
...
/**
 * Ensure a directory exists under .nord/.
 * Creates parent directories as needed.
 *
 * @param relativePath - Path relative to .nord/
 * @param worktreeRoot - Optional worktree root
 * @returns Absolute path to the created directory
 */
export function ensureNordDir(relativePath: string, worktreeRoot?: string): string {
...
export function getWorktreeNotepadPath(worktreeRoot?: string): string {
  return join(getNordRoot(worktreeRoot), 'notepad.md');
}
...
export function getWorktreeProjectMemoryPath(worktreeRoot?: string): string {
  return join(getNordRoot(worktreeRoot), 'project-memory.json');
}
...
export function resolvePlanPath(planName: string, worktreeRoot?: string): string {
  ...
  return join(getNordRoot(worktreeRoot), 'plans', `${planName}.md`);
}
...
export function resolveResearchPath(name: string, worktreeRoot?: string): string {
  validatePath(name);
  return join(getNordRoot(worktreeRoot), 'research', name);
}
...
export function resolveLogsPath(worktreeRoot?: string): string {
  return join(getNordRoot(worktreeRoot), 'logs');
}
...
export function resolveWisdomPath(planName: string, worktreeRoot?: string): string {
  validatePath(planName);
  return join(getNordRoot(worktreeRoot), 'notepads', planName);
}

/**
 * Check if an absolute path is under the .nord directory.
 * @param absolutePath - Absolute path to check
 */
export function isPathUnderNord(absolutePath: string, worktreeRoot?: string): boolean {
  const nordRoot = getNordRoot(worktreeRoot);
  const normalizedPath = normalize(absolutePath);
  const normalizedNord = normalize(nordRoot);
  return normalizedPath.startsWith(normalizedNord + sep) || normalizedPath === normalizedNord;
}

/**
 * Ensure all standard .nord subdirectories exist.
 */
export function ensureAllNordDirs(worktreeRoot?: string): void {
  const nordRoot = getNordRoot(worktreeRoot);
  const subdirs = ['', 'state', 'plans', 'research', 'logs', 'notepads', 'drafts'];
  for (const subdir of subdirs) {
    const fullPath = subdir ? join(nordRoot, subdir) : nordRoot;
    if (!existsSync(fullPath)) {
      try {
        mkdirSync(fullPath, { recursive: true });
      } catch (err) {
...
export function isValidTranscriptPath(transcriptPath: string): boolean {
  ...
  // Allowed: [$CLAUDE_CONFIG_DIR|~/.claude], ~/.nord/..., system temp dir
 ...
 * Path: {nordRoot}/state/sessions/{sessionId}/{mode}-state.json
 *
 * @deprecated Use resolveSessionStatePaths instead.
 ...
 *
 * Fields:
 *  - `sessionScoped`: `.nord/state/sessions/{sessionId}/{name}.json` (or empty when no sid).
 *  - `legacy`: `.nord/state/{name}.json` — preserved for backwards-compat reads.
 ...
 * fallback, write session-only). When migrate=true OR `NORD_MIGRATE_LEGACY_STATE=1`
...
export function resolveSessionStatePaths(
  ...
  validateSessionId(sessionId);
  const sessionScoped = resolveNordPath(`state/sessions/${sessionId}/${normalizedName}.json`, worktreeRoot);
...
export function isLegacyStateMigrationEnabled(): boolean {
  return process.env.NORD_MIGRATE_LEGACY_STATE === '1';
}

/**
 * Get the session state directory path.
 * Path: {nordRoot}/state/sessions/{sessionId}/
 *
 * @param sessionId - Session identifier
 * @param worktreeRoot - Optional worktree root
...
export function getSessionStateDir(sessionId: string, worktreeRoot?: string): string {
  validateSessionId(sessionId);
  return join(getNordRoot(worktreeRoot), 'state', 'sessions', sessionId);
}
...
export function listSessionIds(worktreeRoot?: string): string[] {
  const sessionsDir = join(getNordRoot(worktreeRoot), 'state', 'sessions');
 ...
 *
 * This ensures .nord/ state is always written at the worktree root,
...
export function resolveToWorktreeRoot(directory?: string): string {
  // The resolved root feeds BOTH on-disk `.nord/` placement AND, under
  // NORD_STATE_DIR, the centralized-state *identity* (getProjectIdentifier).
  // The #3349 submodule→superproject climb exists ONLY to place `.nord/` at the
  // superproject working tree; it must NOT change a submodule's centralized
  // identity (that contract is documented on getProjectIdentifier/getNordRoot).
  // So when NORD_STATE_DIR is set — where on-disk placement is moot and identity
 ...
 * This prevents .nord/state/ from being created in subdirectories (#576)
...
export function validateWorkingDirectory(workingDirectory?: string): string {
  ...
  // never the subdirectory, to prevent .nord/ creation in subdirs (#576).
  return trustedRoot;
}
