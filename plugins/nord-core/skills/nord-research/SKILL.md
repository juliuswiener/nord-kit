...
**Scope: CODEBASE ONLY.** This skill investigates local source code via file reads, grep, bash, and git. It does NOT search the web. For web/ideation research use `ideation-lab`. For architectural audit with security/perf/CI lanes use `multi-agent-codebase-audit`.
...
| Full pre-release health audit (12+ lanes) | `multi-agent-codebase-audit` |
| Quick single-pass critique | `scrutinizing-projects` |
| Web / industry / concept research | `ideation-lab` |
