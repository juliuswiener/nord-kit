...
/**
 * Resolve the NORD global configuration/cache directory under the active Claude
 * config dir. This keeps hook/updater/HUD caches aligned with CLAUDE_CONFIG_DIR
 * instead of mixing in ~/.nord.
 */
export function getNordConfigDir(): string {
  return join(getClaudeConfigDir(), '.nord');
}

/** Resolve the canonical update-check cache file path. */
export function getUpdateCheckCachePath(): string {
  return join(getNordConfigDir(), 'update-check.json');
}
