...
/**
 * LSP Go to Implementation Tool - Jump to the concrete implementations of an abstract symbol
 */
export const lspGotoImplementationTool: ToolDefinition<{
  file: z.ZodString;
  line: z.ZodNumber;
  character: z.ZodNumber;
}> = {
...
/**
 * LSP Prepare Call Hierarchy Tool - Resolve the symbol a call hierarchy applies to
 */
export const lspPrepareCallHierarchyTool: ToolDefinition<{
  file: z.ZodString;
  line: z.ZodNumber;
  character: z.ZodNumber;
}> = {
...
/**
 * LSP Incoming Calls Tool - Who calls this function
 */
export const lspIncomingCallsTool: ToolDefinition<{
  file: z.ZodString;
  line: z.ZodNumber;
  character: z.ZodNumber;
}> = {
  ...
  schema: {
    file: z.string().describe('Path to the source file'),
    line: z.number().int().min(1).describe('Line number (1-indexed)'),
    character: z.number().int().min(0).describe('Character position in the line (0-indexed), on the function name')
  },
  handler: async (args) => {
    const { file, line, character } = args;
...
/**
 * LSP Outgoing Calls Tool - What this function calls
 */
export const lspOutgoingCallsTool: ToolDefinition<{
  file: z.ZodString;
  line: z.ZodNumber;
  character: z.ZodNumber;
}> = {
...
export const lspDiagnosticsTool: ToolDefinition<{
  ...
  handler: async (args) => {
    ...
    return withLspClient(file, 'diagnostics', async (client) => {
      ...
      let diagnostics = await client!.collectDiagnostics(file, 30_000);
