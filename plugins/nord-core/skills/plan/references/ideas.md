---
name: brainstorm
description: "Generative idea-exploration roundtable (discuss<->research rounds) growing a shared idea board, then a plan --deep-ready brief. Use for 'explore/generate ideas for X'; generative not ranked/critique. Hand output to plan --deep."
triggers:
  - ideate
  - generate ideas
  - explore ideas
...
SYNTHESIZE (1 agent)
  └─ themes, most-promising (with research backing), most-novel, tensions, decisions-needed, plan_brief
```
...
| Pressure-test / cull a set of candidate ideas (kill the weak) | `plan --stage shortlist` |
| Turn a chosen direction into a consensus plan | `plan --deep` / `planner` |
| Pin vague requirements into a spec via Q&A | `deep-interview` |
| Pure web research report on a known question | `deep-research` |

The natural chain: **ideation-lab → pick a direction → `plan --deep`**. Adversarial-brainstorm is the opposite tool — reach for it only once you have candidates and want them attacked.
...
Or just run the full thing, then react and hand the `plan_brief` onward.
...
4. Offer the **`plan_brief`** as the handoff: "want me to pipe this into `plan --deep`?"
5. The full **board** and **research** are available on request — don't dump them unless asked; lead with the map.

Do not rank-and-cull in the presentation — that is `plan --stage shortlist`'s job, offered as an optional next step if the user wants the space narrowed before planning.
...
- **Ends in a map + a handoff, not a winner.** The synthesis organizes and illuminates; choosing and planning is the user's next move via `plan --deep`.

## Honest limitations

- **Not a feasibility or decision tool.** It widens, it does not narrow. If you need the space culled, follow with `plan --stage shortlist`.
