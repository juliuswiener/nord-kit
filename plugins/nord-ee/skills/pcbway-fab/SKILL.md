---
name: pcbway-fab
description: "PCBWay PCB fab + turnkey/consigned assembly: design rules, ordering. Triggers: PCBWay, turnkey assembly, non-LCSC parts, globally-sourced assembly, PCBWay vs JLCPCB."
...
| Skill | Purpose |
|-------|---------|
| `kicad-analyze` | Read/analyze KiCad project files, DFM scoring |
...
| `lcsc-search` | Search LCSC (production sourcing, JLCPCB parts) |
| `jlcpcb-fab` | JLCPCB PCB fabrication & assembly (primary alternative) |
...
For gerber export settings and CPL format, see the `bom-manager` skill — both JLCPCB and PCBWay use the same formats.
...
3. **Export CPL** (centroid/placement file) as CSV (see `bom-manager` skill for format)
