---
name: kicad-docs
description: "Generate engineering docs from KiCad: HDD, CE technical file, ICD, design review + manufacturing packages; render schematic/PCB SVGs and block diagrams to PDF. Triggers: 'generate documentation/report', HDD, CE file, ICD, render schematic/layout, block diagram, generate PDF."
...
| Skill | Relationship |
|-------|-------------|
| `kicad-analyze` | Produces schematic/PCB/thermal analysis JSON consumed by scaffolds |
| `emc-precheck` | Produces EMC analysis JSON for EMC sections |
| `spice-sim` | SPICE simulation results appear in analog design sections |
| `bom-manager` | BOM data appears in BOM summary sections |

Run the `kicad-analyze` skill's analyzers first, then `emc-precheck` and `spice-sim` if available. The scaffold auto-runs `kicad-analyze` and `emc-precheck` analyses when source files are present, so manual pre-analysis is only needed for SPICE.
