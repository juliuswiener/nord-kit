...
**Automated sync (preferred):** If the `digikey-search` skill is installed, run `sync_datasheets.py` on the schematic. This should have been done in the workflow's Step 3 (see SKILL.md). If not done yet, run it now:
...
2. Use the `digikey-search` skill to search by MPN and download
