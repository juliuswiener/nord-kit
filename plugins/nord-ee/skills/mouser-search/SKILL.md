---
name: mouser-search
description: "Search Mouser for components: pricing, stock, datasheets. Triggers: Mouser, DigiKey out of stock, cross-distributor price compare, parts DigiKey lacks."

---

# Mouser Electronics Parts Search & Analysis

Mouser is the **secondary source for prototype orders** — use when DigiKey is out of stock or has worse pricing. For production orders, see `lcsc-search`/`jlcpcb-fab`. For BOM management and export workflows, see `bom-manager`. For datasheets, prefer DigiKey's API (direct PDF links) — Mouser blocks automated PDF downloads.
