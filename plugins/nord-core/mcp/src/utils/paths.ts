...
function prefersXdgNordDirs(): boolean {
...
/**
 * Legacy global NORD directory under the user's home directory.
 */
export function getLegacyNordDir(): string {
  return join(getUserHomeDir(), '.nord');
}

/**
 * Global NORD config directory.
 *
 * Precedence:
 * 1. NORD_HOME (existing explicit override)
 * 2. XDG-aware config root on Linux/Unix
 * 3. Legacy ~/.nord elsewhere
 */
export function getGlobalNordConfigRoot(): string {
  const explicitRoot = process.env.NORD_HOME?.trim();
  if (explicitRoot) {
    return explicitRoot;
  }

  if (prefersXdgNordDirs()) {
    return join(getConfigDir(), 'nord');
  }

  return getLegacyNordDir();
}

/**
 * Global NORD state directory.
 *
 * When NORD_HOME is set, preserve that existing override semantics by treating
 * it as the shared root and resolving state beneath it.
 */
export function getGlobalNordStateRoot(): string {
  const explicitRoot = process.env.NORD_HOME?.trim();
  if (explicitRoot) {
  ...
  if (prefersXdgNordDirs()) {
    return join(getStateDir(), 'nord');
  }

  return join(getLegacyNordDir(), 'state');
}

export function getGlobalNordConfigPath(...segments: string[]): string {
  return join(getGlobalNordConfigRoot(), ...segments);
}

export function getGlobalNordStatePath(...segments: string[]): string {
  return join(getGlobalNordStateRoot(), ...segments);
}

export function getLegacyNordPath(...segments: string[]): string {
  return join(getLegacyNordDir(), ...segments);
}
...
export function getGlobalNordConfigCandidates(...segments: string[]): string[] {
  if (process.env.NORD_HOME?.trim()) {
    return [getGlobalNordConfigPath(...segments)];
  }

  return dedupePaths([
    getGlobalNordConfigPath(...segments),
    getLegacyNordPath(...segments),
  ]);
}

export function getGlobalNordStateCandidates(...segments: string[]): string[] {
  const explicitRoot = process.env.NORD_HOME?.trim();
  if (explicitRoot) {
    return dedupePaths([
      getGlobalNordStatePath(...segments),
  ...
  return dedupePaths([
    getGlobalNordStatePath(...segments),
    getLegacyNordPath('state', ...segments),
  ]);
}
 ...
 *
 * Structure: <configDir>/plugins/cache/nord/oh-my-claudecode/
 */
export function getPluginCacheBase(): string {
  return join(getClaudeConfigDir(), 'plugins', 'cache', 'nord', 'oh-my-claudecode');
}
