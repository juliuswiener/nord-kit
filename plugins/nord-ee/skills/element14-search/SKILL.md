---
name: element14-search
description: "Search Newark/Farnell/element14 for components: pricing, stock, datasheets (free API). Triggers: Newark, Farnell, element14, non-US/regional distributor, cross-region price compare."
...
| Skill | Purpose |
|-------|---------|
| `kicad-analyze` | Schematic analysis — extracts MPNs for part lookup |
| `bom-manager` | BOM management — orchestrates sourcing across distributors |
| `spice-sim` | Uses element14 parametric data for behavioral SPICE models |
...
For BOM management and export workflows, see `bom-manager`.
