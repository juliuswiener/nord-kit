...
//
// In the MCP server plugin context @ast-grep/napi MUST be installed globally
// (`npm install -g @ast-grep/napi`); it is resolved via the NODE_PATH that the
// bundle's startup banner derives from `npm root -g`. The plugin payload ships
// no node_modules of its own, so without that global install these tools are
// dead on every machine — which is exactly what was measured on 2026-08-10,
// where the package was present in the source repo and nowhere else.
// See docs/TOOL-MAP.md §3 and TOOLING.md.
 ...
 * Only enforced when NORD_RESTRICT_TOOL_PATHS=true.
 *
  ...
  if (rel.startsWith("..") || isAbsolute(rel)) {
    throw new Error(
      `Path restricted: '${inputPath}' is outside the project root '${projectRoot}'. ` +
        `Disable via security.restrictToolPaths in .claude/nord.jsonc or unset NORD_SECURITY.`,
    );
  }
...
function toLangEnum(
  ...
  if (!lang) {
    throw new Error(
      `Language "${language}" is not available in this @ast-grep/napi build. ` +
        `Available: ${availableLanguages(sg).join(", ")}. ` +
        `The others need a separate @ast-grep/lang-* package registered via registerDynamicLanguage().`,
    );
  }
  return lang;
}
...
/**
 * Which languages the installed @ast-grep/napi can actually parse.
 *
 * The set is version-dependent, and the difference is not cosmetic — measured
 * 2026-08-10:
 *
 *   0.31.1 (what package.json pins)  24 languages, incl. Go, Python, Rust
 *   0.45.1 (what `npm i -g @ast-grep/napi` gives you today)   5: Html, JavaScript, Tsx, Css, TypeScript
 *
 * From 0.45 the grammars moved into separate `@ast-grep/lang-*` packages that
 * must be registered via registerDynamicLanguage(). So a plain unpinned global
 * install silently removes 19 languages, and every affected call used to come
 * back as "No matches found" rather than as an error. Pin the global install to
 * the version package.json declares.
 *
 * Read the available set off the module instead of hardcoding a second list,
 * which would rot on the next version either way.
 */
function availableLanguages(sg: typeof import("@ast-grep/napi")): string[] {
  return Object.getOwnPropertyNames(sg.Lang ?? {})
    .map((k) => k.toLowerCase())
    .sort();
}
...
Note: Patterns must be valid AST nodes for the language.`,
  ...
  handler: async (args) => {
    ...
    try {
      ...
      // Resolve the language ONCE, outside the loop. It used to be resolved per
      // file inside the try/catch below, so an unavailable language threw on
      // every file and was swallowed every time — the tool then reported
      // "Searched 10 go file(s) / No matches found", which reads as a valid
      // answer rather than as "this build cannot parse Go".
      let langEnum: import("@ast-grep/napi").Lang;
      try {
      ...
      for (const filePath of files) {
        ...
        try {
          ...
          // Skip files that fail to parse, but keep count: "nothing matched" and
          // "nothing could be read" are different answers and must not look alike.
          parseFailures++;
        }
      }
        ...
        if (parseFailures === files.length) {
          return {
            content: [
              {
                type: "text" as const,
                text: `Could not parse any of the ${files.length} ${language} file(s) in ${path}. This is a parser failure, not an empty result — the pattern was never evaluated.`,
              },
            ],
          };
        }
...
IMPORTANT: dryRun=true (default) only previews changes. Set dryRun=false to apply.`,
  ...
  handler: async (args) => {
    ...
    try {
      ...
      // Same hoist as in ast_grep_search: an unavailable language must fail once
      // and say so, not be swallowed per file and reported as "0 replacements".
      let langEnum: import("@ast-grep/napi").Lang;
      try {
        langEnum = toLangEnum(sg, language);
      } catch (error) {
