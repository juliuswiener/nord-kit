...
| Pressure-test / cull a set of candidate ideas (kill the weak) | `brainstorm-adversarial` |
...
Do not rank-and-cull in the presentation — that is `brainstorm-adversarial`'s job, offered as an optional next step if the user wants the space narrowed before planning.
...
- **Not a feasibility or decision tool.** It widens, it does not narrow. If you need the space culled, follow with `brainstorm-adversarial`.
