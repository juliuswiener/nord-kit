...
description: Use this agent for world-class Python debugging — hypothesis-driven investigation of any hard Python bug, including intermittent production failures, memory leaks, and async or threading race conditions. Triggers on requests like debug this Python error, 500s under load, memory keeps growing, or a race condition I cannot reproduce. <example> User reports a FastAPI app returning intermittent 500s under high load after a DB pool config change; the agent forms hypotheses about pool exhaustion, analyzes evidence, and finds the root cause. </example>
model: sonnet
color: red
---
